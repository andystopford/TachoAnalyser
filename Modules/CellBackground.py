...
class CellBackground(QtGui.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        # TODO clear this on year change
        ...
        rect = QtCore.QRect(QtCore.QPoint(option.rect.x(), option.rect.y()+cell_y), size)

        if index.row() == self.row and index.column() == self.col:
            wtd_triangle = QtGui.QPolygon(3)
            wtd_triangle.setPoint(0, QtCore.QPoint(option.rect.x()+cell_x, option.rect.y()))  # top right
            wtd_triangle.setPoint(1, QtCore.QPoint(option.rect.x(), option.rect.y()))
            wtd_triangle.setPoint(2, QtCore.QPoint(option.rect.x(), option.rect.y()+cell_y))  # bottom left

            hgv_triangle = QtGui.QPolygon(3)
            hgv_triangle.setPoint(0, QtCore.QPoint(option.rect.x() + cell_x, option.rect.y()))
            hgv_triangle.setPoint(1, QtCore.QPoint(option.rect.x() + cell_x, option.rect.y() + cell_y))
            hgv_triangle.setPoint(2, QtCore.QPoint(option.rect.x(), option.rect.y() + cell_y))
            ...
            painter.drawText(QtCore.QPoint(option.rect.x() + (cell_x/4), option.rect.y() + (cell_y * 0.7)), "OK")
            painter.restore()
